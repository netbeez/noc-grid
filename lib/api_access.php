...
// ******************************************************************
	...
	abstract class Api_Access {
		...
		private static function build_url($url_array, $queries_hash){
			...
			$query = http_build_query($queries_hash);
...
// ******************************************************************
    ...
    // NB TESTS
    // ******************************************************************
    class Nb_Tests extends Api_Access {
        // URL: <host>/nb_tests(.*).json

        public static function statuses(array $nb_target_ids, array $agent_ids){
            // URL: <host>/nb_tests/summaries/statuses.json?

            $endpoint = "summaries/" . __FUNCTION__;
            $url = array($endpoint);
            $queries_hash = array('filters[nb_target_ids]' => implode(",", $nb_target_ids), 'filters[agent_ids]' => implode(",", $agent_ids));
            $response = self::make_get_request($url, $queries_hash); //make the request
            return $response;
        }
    }